function DeathPact( event )
	...
	local health_gain = math.floor(target_health*health_gain_pct*round_adjustment)
	...
	local damage_gain = math.floor(target_health * damage_gain_pct*round_adjustment)
