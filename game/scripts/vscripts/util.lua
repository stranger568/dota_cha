...
function Util:MoveHeroToLocation( nPlayerID,vLocation,hObservingTarget)
   ...
   if  hHero then 
     ...
     if Econ.vPlayerData[nPlayerID] and Econ.vPlayerData[nPlayerID].sCurrentStartBlinkEffect  then
        ParticleManager:CreateParticle(Econ.vPlayerData[nPlayerID].sCurrentStartBlinkEffect, PATTACH_ABSORIGIN, hHero)
     else
     ...
     FindClearSpaceForUnit(hHero, vLocation, false)

     if Econ.vPlayerData[nPlayerID] and Econ.vPlayerData[nPlayerID].sCurrentEndBlinkEffect  then
          ParticleManager:CreateParticle(Econ.vPlayerData[nPlayerID].sCurrentEndBlinkEffect, PATTACH_ABSORIGIN_FOLLOW, hHero)
     else
            ...
            if hHero then
                if Util.supposedLocations[nPlayerID] then
                    ...
                    if flDistance>Util.flFenceRadius then
                        print("Автоматическая телепортация за зону выхода")
