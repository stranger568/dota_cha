...
function ActivityModifier:RemoveWearableActivity(hUnit, sItemDef)
    ...
    if hUnit.Activities[sItemDef] then
...
function ActivityModifier:AddWearableActivity(hUnit, translate, sItemDef)
    ...
    for i = 0, 20 do
        ...
        if not hUnit.ActIndexPool[sActIndex] then
            ...
            if _ANIMATION_TRANSLATE_TO_CODE[translate] then
                ...
                --Server:UploadErrorLog("_ANIMATION_TRANSLATE_TO_CODE[translate] is nil, translate:"..translate) 
            end
            break
        end
    end
