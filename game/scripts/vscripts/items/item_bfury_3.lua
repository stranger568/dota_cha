...
function modifier_item_bfury_3:OnCreated()
...
function modifier_item_bfury_3:GetModifierProcAttack_BonusDamage_Physical( params )
	...
	if self.parent:IsTempestDouble() or self.parent:HasModifier("modifier_arc_warden_tempest_double_lua") then return end
	if self.parent:FindAllModifiersByName("modifier_item_bfury_3")[1] == self and not params.target:IsHero() then
		local bonus = 0
		...
		if not self.parent:IsRangedAttacker() then
			return self.quelling_bonus + percentage_damage
		else
			return self.quelling_bonus_ranged + percentage_damage
		end
	end
end
...
function modifier_item_bfury_3:AttackLandedModifier(params)
	...
	if self.parent.split_attack then
	...
	local enemies = FindUnitsInRadius(params.attacker:GetTeamNumber(), target_loc, nil, self.cleave_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_NOT_ATTACK_IMMUNE, FIND_ANY_ORDER, false)
