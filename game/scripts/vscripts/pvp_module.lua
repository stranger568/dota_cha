...
function PvpModule:RoundPrepare(nRoundNumber)
    ...
    if GetMapName()=="halloween_1x8" then
       PvpModule.nBetBonus = PvpModule.nBetBonus * 0.5
    end 
...
function PvpModule:EndPvp(nWinnerTeamID,nLoserTeamID)
     ...
     Timers:CreateTimer({ endTime = 1, 
        callback = function()
            print("Текущий раунд", GameMode.currentRound["nRoundNumber"])
            
            if string.find(GetMapName(),"1x8") then
                if GameMode.currentRound["nRoundNumber"] > 130 then
                    if  GameMode.nRank and GameMode.nRank <= 6 then
                        PvpModule:PunishLoser(nWinnerTeamID,nLoserTeamID)
                    end
                elseif GameMode.currentRound["nRoundNumber"] > 115 then
                    if  GameMode.nRank and GameMode.nRank <= 5 then
                        PvpModule:PunishLoser(nWinnerTeamID,nLoserTeamID)
                    end
                elseif GameMode.currentRound["nRoundNumber"] > 100 then
