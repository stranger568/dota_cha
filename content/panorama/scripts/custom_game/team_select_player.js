    ...
    if (rank_info != null) {
       ...
       {    
            if (rank_info.calibrating_games[3] > 0)
            {
                ...
                $("#EarlyLeave").text = (rank_info.games[3] || 0)
            } 
            else 
            {
                if ( (rank_info.rating_number_in_top != 0 && rank_info.rating_number_in_top != "0" && rank_info.rating_number_in_top <= 10) && (rank_info.mmr[3] || 2500) >= 5420)
               {
                      ...
                      $("#rank_image").style.backgroundImage = 'url("file://{images}/custom_game/ranks/' + GetImageRank(rank_info.mmr[3] || 2500) + '.png")';
               }
                $("#rank_image").style.backgroundSize = "100%" 
                $("#PlayTime").text = (rank_info.mmr[3] || $.Localize("#dota_mind_calibrating"))
                $("#EarlyLeave").text = (rank_info.games[3] || 0)
            }
