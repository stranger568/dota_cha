...
function DrawLotteryFromServer() {
     ...
     var currentIndex = $("#page_lottery").currentIndex
     GameEvents.SendCustomGameEventToServer( "DrawLottery", {playerId:playerId,currentIndex:currentIndex} );
...
function SetFakeCells(number,level,array,itemIndex){
    ...
    for (var i = indexs.length - 1; i >= 0; i--) {
        ...
        var econ_index = CustomNetTables.GetTableValue("econ_rarity", "econ_index");
        var temp=[]
        for(var key in econ_rarity){
          if (econ_rarity[key]==level && econ_index[key]==itemIndex)
          {
            temp.push(key)
          }
        }
...
{   
   if (data.type==1 || data.type==2)
   {
        ...
        if (data.type==1)
        {
            ...
            var moneyValue= parseInt(CustomNetTables.GetTableValue("econ_data", "money_"+steam_id)["money"])-50
            var fragmentValue= parseInt(CustomNetTables.GetTableValue("econ_data", "money_"+steam_id)["fragment"])+parseInt(data.refund)
        ...
        //设置假砖块位置 根据真砖块等级  调整假砖块数量
        SetFakeCells(targetLevel==4?0:1,4,arrayObj.array,data.index)
        SetFakeCells(targetLevel==3?1:2,3,arrayObj.array,data.index)
        SetFakeCells(targetLevel==2?3:4,2,arrayObj.array,data.index)
        SetFakeCells(targetLevel==1?6:7,1,arrayObj.array,data.index)
...
function PlayerMoneyChanged(){

    var playerId = Game.GetLocalPlayerInfo().player_id;
    var steam_id = Game.GetPlayerInfo(playerId).player_steamid;
    steam_id = ConvertToSteamId32(steam_id);

    if (CustomNetTables.GetTableValue("econ_data", "money_"+steam_id))
    {
...
{
    BuildLottery();
    CustomNetTables.SubscribeNetTableListener("econ_data", PlayerMoneyChanged);
